...
| 隧道托管 | 前台服务保活内嵌 `libfrpc.so`（frp 官方 android_arm64 二进制），崩溃自动重启（指数退避） |
...
{ "frp": { "enabled": true, "serverAddr": "<VPS IP>", "serverPort": 7000, "mode": "xtcp" } }
```
...
- 已连接时，主页面仍是原始 DSH Web。移动适配由 **App 在页面加载完成后注入的 `res/raw/mobile.js`** 完成，**不依赖服务器端是否安装 dsh-remote-plugin**——连接官方 DSH Web 与连接带插件的网关表现一致。
