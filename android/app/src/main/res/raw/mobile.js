/**
 * DSH Remote Android —— 移动端页面适配脚本（由 App 在页面加载完成后注入）。
 *
...
(function () {
	...
	/**
	 * React 把当前元素 props 缓存在 __reactProps$*（兼容回退为 fiber.memoizedProps）。
	 * 直接调用官方按钮的 onClick 闭包，就会走与真人点击完全相同的
	 * ctx.layout.toggleSidebar()，不受祖先 visibility/pointer-events 或事件委托影响。
	 */
	...
	/**
	 * 切换官方侧栏，带状态验证与分层重试：
	 *   1. 有 React onClick 闭包时直接调用（最可靠）；
	 *   2. 没有 React 缓存时走标准 HTMLElement.click()；
	 *   3. 280ms 后状态仍未翻转，再补完整 pointer/mouse/click 序列。
	 */
	...
	function toggleSidebar() {
		...
		toggleBusy = true;
		var usedReactHandler = invokeReactOnClick(button);
		if (!usedReactHandler) button.click();
		...
		window.setTimeout(function () {
			...
			if (button2) {
				// 第一轮没有 React 缓存时，再查一次（React 可能刚完成 hydration）；
				// 否则直接用完整事件序列兜底。
				var retriedReact = !usedReactHandler && invokeReactOnClick(button2);
