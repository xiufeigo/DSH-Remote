...
威胁建模：攻击者能扫到的公网面按形态分两档——

- **entry 形态**：攻击者能扫到 `https://<VPS>:8443`，除此之外一无所有；
- **stcp/xtcp 形态（默认推荐）**：VPS 上**不存在任何 DSH 相关的可扫描入口**，
  只剩 frps 控制口（无 token 连控制会话都建不起来）；连接入口只在持有
  访客密钥的设备本地（127.0.0.1）。xtcp 打洞成功后数据端到端直达，
  不经 VPS。
...
2. **自研壳 App**（✅ Android 已交付，见 android/README.md）：
   - ~~WebView 加载网关 + 自签证书锁定（指纹走配对二维码下发）~~ 已实现，
     且升级为「指纹随 visitor 连接串下发、连接前预置锁定」——比配对页下发更早生效
   - ~~STCP 访客模式（frp secret tcp）：VPS 连对外端口都不开~~ 已实现并超出原设计：
     同时支持 stcp 与 xtcp（P2P 打洞优先、失败自动回退中转），App 内嵌
     frpc 二进制以前台服务托管；`dsh-remote visitor` 出码扫码导入
   - iOS 视开发者账号条件（传输协议两端通用，可后置）
3. **推送通知**：会话完成/审批请求到达时推送（依赖壳 App 的前台服务——服务本体已就位）
